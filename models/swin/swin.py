...
# SWIN Block:
        # LayerNorm 
        # Shifted Window based MSA module
        # LayerNorm 
        # 2-layer MLP with GELU non-linearity
        

# print(timm.list_models("swin*", pretrained=True))
model_pretrained = timm.create_model("swin_base_patch4_window7_224", pretrained=True).to('cuda')
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.decoder = nn.Sequential(
                ...
                nn.Upsample(scale_factor=(224 / 9), mode="bilinear", align_corners=True)
            )
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # Segmentation Head
        self.decoder = Decoder(in_channels=768, out_channels=1)
...
if __name__=='__main__':
    ...
    model = model_pretrained(test)
    image = cv2.imread('test.png')
    image = torch.from_numpy(image)
    image = image.unsqueeze(0)
    image = torch.reshape(image, (1, 3, 640, 640))
    image = transforms.Resize((224, 224))(image.float())
    torchvision.utils.save_image(model, 'image.jpg') 