...
from interpretability.project_utils import to_numpy
...
def grad_to_img(img, linear_mapping, smooth=15, alpha_percentile=99.5):
    ...
    # cv2.imwrite('./test.jpg', linear_mapping)
    ...
    # print(rgb_grad[:3].size())
    # print((rgb_grad[:3]).size())
    # print((rgb_grad[3:]+1e-12).size())
    # print(rgb_grad[3:])
    ...
    rgb_grad = np.concatenate([to_numpy(rgb_grad), alpha], axis=0)
    # Reshaping to [H, W, C]
