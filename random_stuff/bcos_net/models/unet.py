...
from models.bcos import BcosConv2d
...
class UpsampleBlock(nn.Module):
    ...
    def __init__(self, ch_in, ch_out=None, skip_in=0, use_bn=True, parametric=False):
        ...
        if parametric:
            ...
            self.up = None
            ch_in = ch_in + skip_in
            self.conv1 = BcosConv2d(inc=ch_in, outc=ch_out, kernel_size=(3, 3),
                                    stride=1, padding=1, max_out=2, b=2,)
            # self.bn1 = nn.BatchNorm2d(ch_out) if use_bn else None
        ...
        self.conv2 = BcosConv2d(inc=conv2_in, outc=ch_out, kernel_size=(3, 3),
                                stride=1, padding=1, max_out=2, b=2,)
...
class Unet(nn.Module):
    ...
    def __init__(self,
        ...
        self.final_conv = BcosConv2d(decoder_filters[-1], classes, kernel_size=(1, 1))
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        with torch.set_grad_enabled(True):
            ...
            targets = torch.empty(1, 3, 512, 512).normal_()

            out = net(batch)
            # print(f'PREDICTIONS OUTPUT : {out.shape}')
